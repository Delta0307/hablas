...
extern "C" __global__ __aicore__ void hablas_hsyr2k_kernel(hablasFillMode_t uplo,
                                                           ...
                                                           int64_t ldc)
#else
extern "C" __global__ __aicore__ void hablas_hsyr2k_kernel(__gm__ half *matrixA,
                                                           __gm__ half *matrixB,
                                                           __gm__ half *matrixC)
#endif
{
#ifdef CAMODEL_PROFILING
    hablasFillMode_t uplo = hablas_fill_lower;
    hablasOperation_t trans = HABLAS_OP_N;
    int64_t N = 999;
    int64_t K = 999;
    half alpha = 1.0;
    int64_t lda = N;
    ...
    for (int i = 0; i < tiles_per_core; ++i)
    {
        ...
        for (int j = 0; j < 2 * k_loop; ++j)
        {

            int64_t k_real = k;
            int64_t k_real_pad = k;
            if ((j == 2 * k_loop - 1 || j == 2 * k_loop - 2) && k_remain)
            {
                k_real = k_remain;
            ...
            if (trans == HABLAS_OP_T)
            {
                if (j & 1)
                {
                    A_ptr = matrixB + row * n * ldb + j / 2 * k;
                    B_ptr = matrixA + col * n * lda + j / 2 * k;
                    LDA = ldb;
                    LDB = lda;
                }
                else
                {
                    A_ptr = matrixA + row * n * lda + j / 2 * k;
                    B_ptr = matrixB + col * n * ldb + j / 2 * k;
                    LDA = lda;
                    LDB = ldb;
                }
                TRANSA = HABLAS_OP_T;
            ...
            else
            {
                if (j & 1)
                {
                    A_ptr = matrixB + j / 2 * k * ldb + row * n;
                    B_ptr = matrixA + j / 2 * k * lda + col * n;
                    LDA = ldb;
                    LDB = lda;
                }
                else
                {
                    A_ptr = matrixA + j / 2 * k * lda + row * n;
                    B_ptr = matrixB + j / 2 * k * ldb + col * n;
                    LDA = lda;
                    LDB = ldb;
                }
                TRANSA = HABLAS_OP_N;
